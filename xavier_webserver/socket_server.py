...
from camera_processor import is_colored_region
...
def main():
    correlator = audio_processor.CrossCorrelator
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while not (maze_1_done and maze_2_done and dual_fates_done):
            ...
            with conn:
                ...
                if msg == MAZE_1_CAM:
                    conn.send(is_colored_region(0))
                elif msg == MAZE_2_CAM:
                    conn.send(is_colored_region(1))
                elif msg == DUAL_FATES:
                    conn.send(correlator.cross_correlate())
